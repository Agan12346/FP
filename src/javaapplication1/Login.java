	...
	public Login() {
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String query = "SELECT * FROM agan3_users WHERE uname = ? and upass = ?;";
				try (PreparedStatement stmt = conn.getConnection().prepareStatement(query)) {
					...
					if (rs.next()) {
						...
						int userId = rs.getInt("uid");// get table column value
						new Tickets(admin, userId); //open Tickets file / GUI interface
